...
FROM (
    SELECT u.user_id, u.user_name, o.order_id,
    SUM(p.product_price) AS total_price,
    ROW_NUMBER() OVER (PARTITION BY u.user_id ORDER BY SUM(p.product_price) DESC) rn
    FROM users u
    JOIN orders o ON u.user_id = o.user_id
    JOIN order_details od ON o.order_id = od.order_id
    JOIN products p ON od.product_id = p.product_id
...
FROM (
    ...
    ROW_NUMBER() OVER (PARTITION BY u.user_id ORDER BY SUM(p.product_price) ASC) rn
    FROM users u
    JOIN orders o ON u.user_id = o.user_id
    JOIN order_details od ON o.order_id = od.order_id
    JOIN products p ON od.product_id = p.product_id
...
FROM (
    ...
    SUM(p.product_price) AS total_price,
    COUNT(p.product_id) AS total_products,
    ROW_NUMBER() OVER (PARTITION BY u.user_id ORDER BY COUNT(p.product_id) DESC) rn
    FROM users u
    JOIN orders o ON u.user_id = o.user_id
    JOIN order_details od ON o.order_id = od.order_id
    JOIN products p ON od.product_id = p.product_id
